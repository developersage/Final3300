...
void DS1621_Init()
{
    char Device = 0x48; //I2C address for DS1621 device
...
{
    char Data_Ret; //declare a variable to return at the end.
    char Device = 0x48;  //I2C address for DS1621 device
    char Cmd = READ_TEMP;  //Read_Temperature command
    ...
    return Data_Ret; //return the given temperature value
}

void DS3231_Read_Time()
{
    char Device = 0x68;  //I2C address for DS3231 device
    char Address = 0x00;  //Value of register 0x00 pointing to "second"
    I2C_Start();                      // Start I2C protocol
    I2C_Write((Device << 1) | 0);     // Device address
    ...
    //read the time from DS3231 chip
    second = I2C_Read(ACK);         
    minute = I2C_Read(ACK);         
    hour = I2C_Read(ACK);
    dow = I2C_Read(ACK);
